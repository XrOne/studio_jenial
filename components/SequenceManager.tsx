...
import { DecoupageTechnique, ShotEntry } from './DecoupageTechnique';
...
const SequenceManager: React.FC<SequenceManagerProps> = ({
  ...
  // === DÉCOUPAGE TECHNIQUE: Convert prompts to ShotEntry for table display ===
  const [shotEntries, setShotEntries] = useState<ShotEntry[]>([]);

  // Initialize shot entries from prompts when in découpage mode
  useEffect(() => {
    if (sequenceMode === 'decoupage' && allPrompts.length > 0 && shotEntries.length === 0) {
      const entries: ShotEntry[] = allPrompts.map((prompt, idx) => ({
        id: `shot-${idx}-${Date.now()}`,
        number: idx + 1,
        valeur: idx === 0 ? "Plan d'ensemble" : idx === allPrompts.length - 1 ? "Gros plan" : "Plan moyen",
        cadrage: 'Face',
        mouvement: 'Fixe',
        action: prompt.substring(0, 50) + (prompt.length > 50 ? '...' : ''),
        prompt: prompt,
      }));
      setShotEntries(entries);
    }
  }, [sequenceMode, allPrompts.length]);

  const handleUpdateShot = (id: string, updates: Partial<ShotEntry>) => {
    setShotEntries(prev => prev.map(shot =>
      shot.id === id ? { ...shot, ...updates } : shot
    ));
  };

  const handleDeleteShot = (id: string) => {
    setShotEntries(prev => prev.filter(shot => shot.id !== id).map((shot, idx) => ({
      ...shot,
      number: idx + 1
    })));
  };
  ...
  const handleAddShot = () => {
    ...
    setShotEntries(prev => [...prev, newShot]);
  };
  ...
  return (
    <div className="bg-[#1f1f1f] border border-gray-700 rounded-2xl p-4 h-full flex flex-col shadow-lg">
      ...
      <div className="overflow-y-auto flex-grow pr-2 -mr-2 max-h-[calc(100vh-280px)] scrollable-panel">
        {/* === DÉCOUPAGE MODE: Show professional table === */}
        {sequenceMode === 'decoupage' && shotEntries.length > 0 ? (
          <DecoupageTechnique
            shots={shotEntries}
