...
const AIEditorModal: React.FC<AIEditorModalProps> = ({ image, onClose, onConfirm, dogma }) => {
  ...
  const handleEdit = async (overridePrompt?: string) => {
    ...
    try {
      ...
      if (styleImage) {
        ...
        // The service expects @image1 (target), @image2 (reference) etc.
        fullPrompt = `${finalPromptText}. Apply the style from @image2 to the edit on @image1.`;
      }
      
      // UPGRADE: Using latest Gemini models
  ...
  const handleConfirm = () => {
    if (editedImage) {
      ...
      // If no edit was performed, confirm with the original image
      onConfirm(image);
    }
  };
            ...
            <div className="w-1/3 flex flex-col gap-4">
                ...
                <div className="mt-auto">
                    <button
                        ...
                        className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg transition-all shadow-lg hover:shadow-xl disabled:bg-gray-600 disabled:cursor-wait ${
                            model === 'nano-pro' 
